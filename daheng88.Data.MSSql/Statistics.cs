    ...
    public class Statistics : IStatistics
    {
        public bool UpdateStatistics(StatisticsInfo statistics)
        {
            string cmdText = @"update [loachs_sites] set 
                                ...
                                TagCount=@TagCount";
            SqlParameter[] prams = {
					                        SqlHelper.MakeInParam("@PostCount", SqlDbType.Int,4,statistics.PostCount),
					                        SqlHelper.MakeInParam("@CommentCount", SqlDbType.Int,4,statistics.CommentCount),
					                        SqlHelper.MakeInParam("@VisitCount", SqlDbType.Int,4,statistics.VisitCount),
					                        SqlHelper.MakeInParam("@TagCount", SqlDbType.Int,4,statistics.TagCount),
                                        };
        ...
        public StatisticsInfo GetStatistics()
        {
            ...
            List<StatisticsInfo> list = DataReaderToList(SqlHelper.ExecuteReader(cmdText));

            if (list.Count == 0)
            {
                SqlHelper.ExecuteNonQuery(insertText);
            }
            list = DataReaderToList(SqlHelper.ExecuteReader(cmdText));
